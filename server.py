...
s.listen()
print("[INICIO] Esperando conexion")
...
def threaded_client(conn, game, spec=False):
    ...
    if not spec:
        ...
        while True:
            ...
            try:
                ...
                if not d:
                    ...
                    if data == "ganador b":
                        bo.winner = "b"
                        print("[JUEGO] Jugador b gano el juego", game)
                    if data == "winner w":
                        bo.winner = "w"
                        print("[G] Jugador w gano el juego", game)

                    if data == "update moves":
                        bo.update_moves()

                    if data.count("nombre") == 1:
                        name = data.split(" ")[1]
                        if currentId == "b":
                            bo.p2Name = name
                        elif currentId == "w":
        ...
        try:
            del games[game]
            print("[Juego] Juego", game, "termino")
        except:
            pass
        print("[DESCONECTADO] Jugador desconectado", name, "Salir del juego", game)
        conn.close()
